...
export const matchInlineFields = (regex: RegExp, line: string, attribute: string, input: string): { oldField: string, newField: string }[] => {
    ...
    const newContent = content.map((line, i) => {
        if (frontmatter && i >= start.line && i <= end.line) {
            ...
            if (fR?.groups && Object.keys(fR.groups).every(j => fieldComponents.includes(j))) {
                ...
                newFields.push(...matchInlineFields(inSentenceRegexBrackets, encodedLine, attribute, encodedInput))
                newFields.push(...matchInlineFields(inSentenceRegexPar, encodedLine, attribute, encodedInput))
