...
import BaseModal, { MultiFilesBaseModal } from "../baseFieldModals/BaseModal";
...
export class MultiFilesInputModal extends MultiFilesBaseModal {
    private templateValues: Record<string, string> = {};
    private renderedValue: TextAreaComponent;
    private newValue: string;
    private value: string;
    ...
    public async save(): Promise<void> {
        await Promise.all(this.files.map(async f => {
            await postValues(this.plugin, [{ indexedPath: this.indexedPath || this.field.id, payload: { value: this.newValue } }], f, this.lineNumber, this.asList, this.asBlockquote)
        }))
        this.close()
    }
} 